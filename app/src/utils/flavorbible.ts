...
export const getStructuredDataByRootUsingFirstPhrase = (items: string[]) =>{
 ...
 export const filterCleanedUpArray = (cleanedUpResults, query) => {
  // Return an object with the same shape as cleanUpResults (keyed by field),
  // so callers can index by field name whether or not a query was applied.
  const filtered = {}
  Object.keys(cleanedUpResults).forEach((field)=>{
    filtered[field] = cleanedUpResults[field].filter(item => item.includes(query))
  })
  return filtered
 }